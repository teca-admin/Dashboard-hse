...
const App: React.FC = () => {
  ...
  const loadData = useCallback(async (isBackground = false) => {
    ...
    try {
      ...
      setState(prev => ({ 
        ...prev, 
        loading: false, 
        error: err.message || "Erro inesperado na comunicação com a base de dados." 
      }));
    }
  }, []);
  ...
  return (
    <div className="h-screen bg-white flex flex-col p-4 lg:px-8 lg:py-4 overflow-hidden">
      {/* Executive Header with Navigation */}
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="hidden xl:flex items-center gap-3">
             <div className="px-3 py-1.5 bg-indigo-50 border border-indigo-100 rounded-lg">
                <span className="text-[10px] font-bold text-indigo-700 uppercase tracking-widest">SIA | Safety Intelligence v3.5</span>
             </div>
          </div>
        </div>
      </header>
